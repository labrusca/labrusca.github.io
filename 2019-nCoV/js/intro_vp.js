...
$(document).ready(function() {
    ...
    __data_arr.map(function(d) {
      ...
      var _html = `
        <div class="placeItemWrap ${d.area == '湖北' ? 'current' : ''}" area="${d.area}">
          <div class="clearfix placeItem placeArea ${d.citys.length <= 0 ? 'no-sharp' : ''}">
            <h2 class="blue">${d.area}</h2>
            <div class="add ac_add">0</div>
      ...
      `;
      $el.append(_html);
      var $_closestDom = $('.placeItemWrap[area="' + d.area + '"]');
    ...
    $.ajax({
      ...
      success: (res) => {
        ...
        if (foreignArray.length > 0) {
          $('.places')
            .append(`<div class="up-tips up-gnbl">海外国家<span class="datas">确诊<span class="hw_confirm">0</span>例，死亡<span class="hw_dead">0</span>例</span></div><div class="clearfix placeItem nav abroad">
          <h2>地区</h2>
          <div>确诊</div>
          <div>治愈</div>
          <div>死亡</div>
  ...
  function areaAdd() {
    ...
    if (parseInt(nowTime.hour) <= 0 && parseInt(nowTime.minutes) <= 30) {
      return;
    }
    $.ajax({
      url: 'https://view.inews.qq.com/g2/getOnsInfo?name=wuwei_ww_area_adds',
      dataType: 'jsonp',
      jsonp: 'callback',
      scriptCharset: 'UTF-8',
      success: (res) => {
        ...
        for (var i in data) {
          var item = data[i];
          var item_confirm = parseInt(item.confirm);
          if (item.country === '中国') {
            var $this_area = $(`.placeItemWrap[area="${item.area}"]`);
            var $this_area_add = $this_area.find('.placeArea .ac_add');
            var this_area_confirm_add = parseInt($this_area_add.text());
            $this_area_add.text(this_area_confirm_add + item_confirm);
            var $this_city = $(`.placeItem[city="${item.city}"]`);
            $this_city.find('.ac_add').text(item_confirm);
          } else {
            var $this_country = $(`.placeItemWrap[country="${item.country}"]`);
            $this_country.find('.ac_add').text(item_confirm);
          }
        }
      },
      error: (err) => {
  ...
  (function() {
    $.ajax({
      ...
      success: (res) => {
        ...
        $('.marquee-tab .text')
          .html(hintWords)
          .css('width', Math.max(100, (hintWords.length / 14) * 100) + '%');
        map();
        areaMap();
        $('body').on('click', '.placeArea', function() {
          $(this)
